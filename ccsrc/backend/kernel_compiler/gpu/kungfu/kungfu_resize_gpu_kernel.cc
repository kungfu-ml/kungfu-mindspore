...
class cuda_var
{
  ...
  public:
    ...
    T get() const
    {
        ...
        if (ret != cudaSuccess) {
            MS_LOG(ERROR) << "cudaMemcpy failed, ret[" << static_cast<int>(ret)
...
bool KungFuResizeGpuKernel::Launch(const std::vector<AddressPtr> &inputs,
...
MS_REG_GPU_KERNEL_REGULAR(KungFuResize,
                          KernelAttr()
