...
class KungFuDataOp : public ParallelOp, public RandomAccessOp
{
  public:
    class Builder
    {
      public:
        // Constructor for Builder class of KungFuDataOp
        Builder();
        ...
        // @param std::shared_ptr<KungFuDataOp> *op - DatasetOp
        // @return Status The status code returned
        Status Build(std::shared_ptr<KungFuDataOp> *op);
    ...
    // Constructor
    // @param const std::string &usage - Usage of this dataset, can be 'train',
    // 'test' or 'all'
    ...
    // @param std::unique_ptr<DataSchema> data_schema - the schema of the mnist
    // dataset
    // @param td::unique_ptr<Sampler> sampler - sampler tells KungFuDataOp what
    // to read
    KungFuDataOp(const std::string &usage, int32_t num_workers,
                 int32_t rows_per_buffer, std::string folder_path,
                 int32_t queue_size, std::unique_ptr<DataSchema> data_schema,
                 std::shared_ptr<SamplerRT> sampler);

    // Destructor.
    ~KungFuDataOp() = default;

    // Worker thread pulls a number of IOBlock from IOBlock Queue, make a buffer
    // and push it to Connector
    // @param int32_t worker_id - id of each worker
    // @return Status The status code returned
    Status WorkerEntry(int32_t worker_id) override;

    // Main Loop of KungFuDataOp
    // Master thread: Fill IOBlockQueue, then goes to sleep
    // Worker thread: pulls IOBlock from IOBlockQueue, work on it then put
    // buffer to mOutConnector
    // @return Status The status code returned
    Status operator()() override;

    // Method derived from RandomAccess Op, enable Sampler to get all ids for
    // each class
    // @param (std::map<uint64_t, std::vector<uint64_t >> * map - key label, val
    // all ids for this class
    // @return Status The status code returned
    Status GetClassIds(
        std::map<int32_t, std::vector<int64_t>> *cls_ids) const override;
    ...
    // @param count output arg that will hold the minimum of the actual dataset
    // size and numSamples
    // @return
    static Status CountTotalRows(const std::string &dir,
  ...
  private:
    ...
    // @return Status The status code returned
    Status LoadTensorRow(row_id_type row_id, const MnistLabelPair &mnist_pair,
                         TensorRow *row);

    // @param const std::vector<int64_t> &keys - keys in ioblock
    // @param std::unique_ptr<DataBuffer> db
    // @return Status The status code returned
    Status LoadBuffer(const std::vector<int64_t> &keys,
    ...
    // @return Status The status code returned
    Status TraversalSampleIds(const std::shared_ptr<Tensor> &sample_ids,
    ...
    // @return Status The status code returned
    Status CheckImage(const std::string &file_name, std::ifstream *image_reader,
                      uint32_t *num_images);
    ...
    // @return Status The status code returned
    Status CheckLabel(const std::string &file_name, std::ifstream *label_reader,
                      uint32_t *num_labels);
    ...
    // @return Status The status code returned
    Status ReadImageAndLabel(std::ifstream *image_reader,
                             std::ifstream *label_reader, size_t index);
