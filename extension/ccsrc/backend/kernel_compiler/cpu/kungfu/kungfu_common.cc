...
bool show_kungfu_debug_log()
{
    return getenv("KUNGFU_MINDSPORE_DEBUG") != nullptr;
}
...
int kungfu_current_rank()
{
    return _kungfu_peer->Rank();
}

int kungfu_current_cluster_size()
{
    return _kungfu_peer->Size();
}
...
REGISTER_PYBIND_DEFINE(KungFu_, ([](py::module *m) {
                           ...
                           m->def("kungfu_current_cluster_size",
                                  &kungfu_current_cluster_size);
                       }));
}  // namespace kernel
}  // namespace   mindspore
